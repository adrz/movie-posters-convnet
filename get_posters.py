...
def get_url_imgs(url_start, year):
    ...
    df['year'] = year
    df['url_imgs'] = url_start + str(year) + '/posters/' + df.html_link.str.replace('.html','.jpg')
    return df



def download_posters(df, convert_location):
    #convert_str = '/ldaphome/.../dev/ImageMagick-6.9.8-3/utilities/convert '
    local_image = []
    local_thumb = []
    ...
    for index, row in df.iterrows():
        ...
        check_call(['wget', '-P', folder, row['url_imgs']], stdout=DEVNULL, stderr=STDOUT)
        file_str = folder + row['url_imgs'].split('/')[-1]
        local_image.append(file_str)
        thumbnail_file = './thumbs/' + str(row['year']) + '/' + row['url_imgs'].split('/')[-1]
        #check_call([convert_location, file_str, '-resize 50x50!', thumbnail_file], stdout=DEVNULL, stderr=STDOUT)

        # Downsample posters to a 50x50 pixels (for datavisualization purpose)
        str_system = convert_location + ' ' + file_str + ' -resize 50x50! ' + thumbnail_file
    ...
    # for index, row in df.iterrows():
    #     folder = 'posters/'+str(row.year) + '/'
    #     str_system = 'wget -P ' + folder + ' ' + row.url_imgs
    #     os.system(str_system)
    #     file_str = folder + row.url_imgs.split('/')[-1]
    #     local_image.append(file_str)
    #     thumbnail_file = 'thumbs/' + str(row.year) + '/' + row.url_imgs.split('/')[-1]
...
def main(argv):
    ...
    parser.add_argument('-r', '--range', help="range of years to scrap (ex: 1920-2017)", default="1990-2017")
    parser.add_argument('-o', '--output_file', help="output pickle file used to locate posters file", default='./cnn_posters.p')
    parser.add_argument('-c', '--convert_location', help="location of ImageMagick convert", default='convert')
    ...
    #map(create_folder, folders_to_create)
